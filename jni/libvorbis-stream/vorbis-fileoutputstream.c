 ...
 * Solved bug by Ricardo Galli (gallir@gmail.com): it didn't
 *  read the remaing packets after sending the null buffer during close.
 *  Added  write_pages and
 *
...
void analysis(struct output_stream *optr) {
	...
	while (vorbis_analysis_blockout(&optr->vd, &optr->vb) == 1) {
		...
		while (vorbis_bitrate_flushpacket(&optr->vd, &optr->op)) {
			...
			while(ogg_stream_pageout(&optr->os, &optr->og) > 0) {
				write(optr->fd, optr->og.header, optr->og.header_len);
				write(optr->fd, optr->og.body, optr->og.body_len);
			}
			//while (! ogg_page_eos(&optr->og));
		...
		)
{
	...
	/* JNI field ID's */
	jfieldID channels_field, sample_rate_field, quality_field, bit_rate_field, coder_field;
...
//		if (output_streams[stream_idx].fh == NULL) {
		if (output_streams[stream_idx].used == 0) {
			...
			optr->fd = open(pchars, O_RDWR|O_TRUNC|O_CREAT, 0064);
			//if (optr->fh == NULL) {
			if (optr->fd < 0) {
			...
			//setvbuf(optr->fh, NULL, _IONBF, 0);
			(*env)->ReleaseStringUTFChars(env, path, pchars);
			output_streams[stream_idx].used = 1;
			break;
		}
	}
	...
	bit_rate_field = (*env)->GetFieldID(env, cls, "bitRate", "I");
	coder_field = (*env)->GetFieldID(env, cls, "coder", "I");
	...
	bit_rate = (*env)->GetIntField(env, info, bit_rate_field);
	coder = (*env)->GetIntField(env, info, coder_field);

	/* Choose the coder */
	if (coder == 0) {
		ret = vorbis_encode_init(&optr->vi,optr->channels,sample_rate,-1,bit_rate,-1);
	} else {
	...
	vorbis_comment_add_tag(&optr->vc,"ENCODER","SpokenPic");
		...
		)
{
	...
	if (sidx >= MAX_OUTPUTSTREAMS || sidx < 0 || optr->used == 0) {
...
{
	struct output_stream * optr = &output_streams[sidx];
	if (sidx >= MAX_OUTPUTSTREAMS || sidx < 0 || optr->fd == 0) {
